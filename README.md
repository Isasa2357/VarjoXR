...
- Varjo Native SDK の session / frame / layer / swapchain を独自管理するための入口
- D3D11 backend の入口
- D3D12 backend の入口
- `XRSpace` / `XRPlane` / `XRObject` / `Material`
- 左右 eye 別 texture / pixel shader HLSL の保持API
- CPU RGBA からの texture 作成API
- Varjo view index から `Eye::Left` / `Eye::Right` への解決の土台
- Context / Focus view を texture array slice として submitするための土台
